...
# Returns dictionary from polypharmacy side effects to list of combination ID
def load_se_combo(fname="bio-decagon-combo.csv"):
    se2combo = defaultdict(set)
    fin = open(fname)
    print("Reading: %s" % fname)
    fin.readline()
    for line in fin:
        stitch_id1, stitch_id2, se, _ = line.strip().split(",")
        combo = (stitch_id1, stitch_id2)
